        ...
        <div class="flex items-start py-4">
            <div class="flex items-center justify-between w-full pl-150 pr-40">
                <Title
                    ...
                    class="text-yellow-500 bold py-4"
                    >القصص الاكثر قراءة</Title
                >
                <Title
                    v-if="$page.component === 'Profile/HomePage'"
                    class="text-yellow-500 bold py-4"
                    >  القصص التي تم نشرها</Title
                >

                <Title
                    v-if="user !== null && $page.component === 'Home'"
                ...
                <Title
                    ...
                    class="text-yellow-500 py-4 px-2 rounded-lg"
                >
                    قصصك المفضلة
                </Title>
            </div>
        ...
        <div v-if="props.Cards.data.length" class="px-50">
            <div class="grid grid-cols-3 gap-4">
                <div
                    ...
                    :key="Card.id"
                    class="bg-white rounded-[10px] p-2"
                >
                    <img
                        :src="
                            Card.image
                        ...
                        "
                        alt="صورة الكرت"
                        class="object-cover h-50 w-90 rounded-[10px]"
                    />

                    <div class="p-2">
                        <h3
                            dir="rtl"
                            class="font-bold text-xl mb-2 flex items-center justify-between"
                        >
                        ...
                        <p>
                            listed on {{ Card.created_at.substring(0, 10) }} by
                            <button
                                class="text-link"
                                @click="selectuser(Card.user.id)"
                            >
                                {{ Card.user.name }}
                            </button>
                        </p>
                    ...
                    <div class="flex items-center justify-between p-2 gap-2">
                        <Link
                        :href="route('Card.listen', Card.id)"
